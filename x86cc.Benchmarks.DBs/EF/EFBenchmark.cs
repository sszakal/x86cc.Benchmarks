...
[BenchmarkCategory("Entity Framework DBs")]
[MemoryDiagnoser]
[GcServer(true)]
[Config(typeof(DefaultBenchmarkConfig))]
public abstract class EFBenchmark : IAsyncDisposable
{
    ...
    {
        return SearchAsync(o => o.Status == OrderStatus.Submitted, TestDataCount);
    }
    ...
    public void Delete_Bulk_Setup()
    {
        ...
        CreateBulkAsync(SeedData).GetAwaiter().GetResult();
    }
    ...
    {
        return DeleteBulkAsync(SeedData);
    }
    ...
    protected abstract Task ExecuteCreateAsync(CustomerOrderRoot item);

    protected abstract Task CreateBulkAsync(CustomerOrderRoot[] items);
    ...
    protected abstract Task SearchAsync(Expression<Func<CustomerOrderRoot, bool>> filter, int take);

    protected abstract Task<CustomerOrderRoot?> ExecuteLoadByIdAsync(Guid id);

    protected abstract Task DeleteOneAsync(CustomerOrderRoot order);

    protected abstract Task DeleteBulkAsync(CustomerOrderRoot[] orders);
